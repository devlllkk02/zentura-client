...
function PopupAppointments({ setDisplayPopup, option, current, setCurrent }) {
  //State
  ...
  const handleCreate = async () => {
    try {
      const response = await axios.post("/appointment/create", current);
  ...
  const handleUpdate = async () => {
    await axios.put(`/appointment/update/${current.id}`, current);
    window.location.reload();
  };

  //Delete
  const handleDelete = async () => {
    await axios.delete(`/appointment/delete/${current.id}`);
    window.location.reload();
  };
  return (
    <div className="popup">
...
export default PopupAppointments;
