...
function PopupJob({ setDisplayPopup, option, current, setCurrent }) {
  //State
  ...
  const handleCreate = async () => {
    try {
      const response = await axios.post("/job/create", current);
  ...
  const handleUpdate = async () => {
    await axios.put(`/job/update/${current.id}`, current);
    window.location.reload();
  };

  //Delete
  const handleDelete = async () => {
    await axios.delete(`/job/delete/${current.id}`);
    window.location.reload();
  };
  return (
    <div className="popup">
